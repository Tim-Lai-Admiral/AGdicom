...
- **DICOM 查看器**：元数据表格（模态 / SOP Class / 传输语法 / 行列 / 像素间距 / 患者字段等中文标签）；按 SeriesInstanceUID 聚合并按 InstanceNumber 排序的多切片切换；Canvas 灰度预览（min-max 归一化）；去标识化证据展示；压缩语法 / 损坏文件 / 刷新后等多种降级路径不崩溃；工作台中央查看区内嵌（非模态），打开时聚焦、关闭后焦点还原。
