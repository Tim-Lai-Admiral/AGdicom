...
- **工作台布局（CR-003 / CR-009）**：全屏四区工作台——顶栏（视口工具组 / 筛选 / 搜索 / 导入 / 比较 / 导出 / 面板开关）+ 左栏素材列表（DICOM 卡片可展开 series 切片缩略图）+ 中央统一查看区 + 右栏信息面板（DICOM 显示元数据分组与 W/L 调节，其余素材显示评审面板；左右栏抽屉式开合，0.2s 宽度过渡动画，窄屏侧栏收窄，R-026）。
...
- **图片预览与比较**：单张图片预览支持视口工具 pan/zoom/rotate（CR-009 / R-024）；勾选两张图片并排比较（R-002，上限两张，先选在左）。
