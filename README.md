...
- **素材比较（CR-011 / R-002；CR-012 T-003 / R-029、T-004 / R-030）**：比较为显式模式——普通模式点击素材行 = 中央查看；顶栏「比较」汉字按钮进入比较模式后，素材库先筛选出可比较素材（image + dicom + model，限两个同类型、先选在左，混合类型由同类配对约束拒绝）并提示选择，显式点选两张自动并排比较；退出比较模式（「退出比较」/ Esc / 顶栏「完成」）清空选择并恢复普通列表与查看交互。图片比较：双图等尺寸并排、窗格独立视口变换；DICOM 比较：双系列双窗，切片索引与视图变换两窗同步（W/L 每窗独立）；STL 比较：双 three.js 实例并排各自 fit，任一侧旋转/平移/缩放 → 另一侧相机同步（共享 OrbitControls 目标 + 复制相机变换），加载进度 / 错误降级每窗独立，退出 dispose 清理。
...
- **3D 模型查看器**：STL 解析（分块读取 + 进度）；three.js 渲染（相机自动 fit 包围盒）；OrbitControls 默认映射（左键旋转 / 滚轮缩放 / 右键平移）；WebGL 不可用与损坏文件降级；查看器代码按需加载（React.lazy，TD-002）。双模型比较见上方「素材比较」（CR-012 T-004 / R-030）。
