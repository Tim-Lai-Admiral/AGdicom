...
面向素材整理与评审场景的**本地单机工作台**：导入素材 → 浏览筛选 → 查看（图片并排比较 / DICOM 元数据与切片 / 3D 模型）→ 打标与状态评审 → 导出 JSON 备份。全部数据保存在浏览器本地（localStorage + IndexedDB），**无后端、无账号、不上传任何素材**。

## 功能一览

- **工作台布局（CR-003）**：全屏四区工作台——顶栏（筛选 / 搜索 / 导入 / 比较 / 导出 / 面板开关）+ 左栏素材列表（DICOM 卡片可展开 series 切片缩略图）+ 中央统一查看区 + 右栏信息面板（DICOM 显示元数据分组与 W/L 调节，其余素材显示评审面板；左右栏可折叠，窄屏侧栏收窄）。
...
1. **大于 20MB 的文件字节不持久化**（TD-001 已随 CR-006 解决：≤20MB 文件字节入 IndexedDB，刷新自动恢复预览）：≤20MB 的图片 / DICOM / 3D 模型刷新后无需重新导入；>20MB 或 IndexedDB 不可用的素材刷新后仅剩持久化记录，需重新导入（重导入自动水合恢复预览）或删除该素材；DICOM 元数据已持久化，刷新后仍可查看表格。
