...
/** CR-012 T-002：AIProvider.suggest 允许返回 Promise（远程实现）；Mock 恒为同步，测试按同步断言 */
function suggestSync(asset: Asset): AiSuggestion {
  return mockProvider.suggest(asset) as AiSuggestion
}
...
describe('mockProvider 确定性（R-006）', () => {
  it('同一图片素材两次生成结果完全一致', () => {
    ...
    expect(suggestSync(asset)).toEqual(suggestSync(asset))
  })
...
describe('mockProvider 图片建议（类型+序号）', () => {
  ...
  it('文件名无数字时用资产 ID 的稳定哈希序号（3 位数字，仍确定）', () => {
    const suggestion = suggestSync(makeAsset({ file: { fileName: 'photo.jpg', fileSize: 1, fileType: '' } }))
    expect(suggestion.name).toMatch(/^图片-\d{3}$/)
    expect(suggestSync(makeAsset()).name).toBe(suggestion.name)
...
describe('mockProvider DICOM 建议（序列+切片数）', () => {
  ...
  it('命名为 Modality+序列 UID 前 8 位+切片数，标签含类型/Modality/去标识化/多切片', () => {
    const suggestion = suggestSync(dicomAsset)
  ...
  it('元数据缺失时逐项降级（无 Modality/UID 用 DICOM，切片数未知），仍确定', () => {
    const suggestion = suggestSync(makeAsset({ kind: 'dicom', file: { fileName: 'broken.dcm', fileSize: 10, fileType: '' } }))
    expect(suggestion.name).toBe('DICOM-切片数未知')
    expect(suggestion.tags).toEqual(['DICOM'])
    expect(suggestion.summary).toBe('DICOM 序列，切片数未知。')
    expect(suggestSync(makeAsset({ kind: 'dicom', file: { fileName: 'broken.dcm', fileSize: 10, fileType: '' } }))).toEqual(suggestion)
  })
})
