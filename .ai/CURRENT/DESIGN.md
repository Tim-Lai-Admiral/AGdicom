...
3. DICOM：左栏**独立"患者分组"面板**（CR-008：一次渲染全部患者组，按 R-012 排序；分组头/系列可折叠；点击切片只切换中央查看与高亮，面板不搬家；切片高亮随中央滑动条实时同步 R-022）；同患者缺 UID 文件聚合为"未知系列"；**行缩略图与切片缩略图为真实首帧像素**（解析后生成，会话级缓存；未解析/压缩显示 SVG 占位）；切片导航为**滑动条**（rec 样式，含计数读数）；右栏窗宽窗位滑杆（C:-1000..1000 / W:1..4000）与 6 预设（Lung/Mediastinum/Bone/Brain/Liver/S.Tissue）实时调窗；测量工具拖拽绘制（Mock，PixelSpacing 可用时确定性 mm，否则 px 模拟值）。
