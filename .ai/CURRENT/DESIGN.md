...
3. DICOM：左栏按患者组展开（姓名+ID 分组 → series → 切片缩略图，点击切换）；**行缩略图与切片缩略图为真实首帧像素**（解析后生成，会话级缓存；未解析/压缩显示 SVG 占位）；同患者缺 UID 文件聚合为"未知系列"；切片导航为**滑动条**（rec 样式，含计数读数）；右栏窗宽窗位滑杆（C:-1000..1000 / W:1..4000）与 6 预设（Lung/Mediastinum/Bone/Brain/Liver/S.Tissue）实时调窗；测量工具拖拽绘制（Mock，PixelSpacing 可用时确定性 mm，否则 px 模拟值）。
