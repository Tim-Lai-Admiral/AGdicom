...
- 步数软监控：Expected 默认 20；达到上限 30 步时停止，写入检查点 `.ai/CHANGES/<CR-ID>/TASKS/T-XXX-CHECKPOINT.md`（Completed / Current approach / Failed approaches / Remaining / Blocker / Recommendation），并在报告中说明，等待协调者决定。
...
- 提交完成后：`git push -u origin <分支>` → `gh pr create --base master`（title 用首个 commit 主题；**body 写概要：实现摘要 + 验证结果 + 任务卡路径**）→ PR 链接写入 Builder result；失败则如实报告（规则 6）。
